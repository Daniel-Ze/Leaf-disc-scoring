...
def usage():
    print("Usage: python -i /path/to/images/ -m or -c -l bla,blubb,tralala -e .jpg")
    print("-h print this help")
    print("-i input folder for image_sorter2")
    print("-m move files")
    print("-c copy files")
    print("-l classes to sort images into: bla,blubb,tralala")
    print("-e file extensions that are accepted: .jpg,.png")
    exit()

# the folder in which the pictures that are to be sorted are stored
# don't forget to end it with the sign '/' !
#input_folder = '/Users/daniel/PostDoc/Programs/CNN/leaf_discs_for_classes/score9_LDA_Plate1_II_s10_2018-74-23_Class9.jpg_slice/'

# the different folders into which you want to sort the images, e.g. ['cars', 'bikes', 'cats', 'horses', 'shoes']
#labels = ["back", "spo", "no_spo"]
...
df_path = '/Users/daniel/PostDoc/Programs/image-sorter2/follow.txt'
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "cmhi:l:e:", ["cpoy", "move", "help", "input=", "labels=", "extension="])
    ...
    if file_extensions == []:
        print("[error]\tNo extensions")
        usage()

    if copy_or_move == "":
        print("[error]\tNo copy or move")
        usage()

    print("[info]\tLabels:       " + str(labels))
    print("[info]\tExtensions:   " + str(file_extensions))
    print("[info]\tAction:       " + copy_or_move)
    ...
    if file_names == []:
        print("[error]\tCheck file extensions")
        usage()
